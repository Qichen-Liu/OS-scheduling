...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->priority = 0; // set default priority to 0
  p->tickets = 10; // set default ticket to 10
  release(&ptable.lock);
...
void scheduler(void) {
  ...
  for (;;) {
    ...
    if(totalTickets > 0){
      uint next = next_random();
      uint lottery = next % totalTickets;

      // Loop over process table looking for process to run.
      // edit proc stru, add nice value
      // add syscall for sorting
      // run nice sort array instand 
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if (p->state != RUNNABLE)
          continue;
        ticketsTravelled += p->tickets;
        if(ticketsTravelled < lottery){
          continue;
        }
...
// nice function to assign and change nice value
int nice(int pid, int priority){
  struct proc *p;
  if (priority < -20 || priority > 19){
    cprintf("Invalid nice (-20-19)!\n");
    return 0;
  }
  acquire(&ptable.lock);
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
